...
import {
  Users, Calendar, FileText, Activity, Plus,
...
export default function ParentDashboard() {
  ...
  return (
    <DashboardShell portalLabel="Parent portal" nav={parentNav}>
        ...
        <>
          ...
          <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 rounded-3xl border border-[var(--border)] bg-[var(--card)] p-6 shadow-sm">
              ...
              {children.length === 0 ? (
                <EmptyState icon={Users} title="No children yet" description="Add your first child profile to begin tracking their journey." cta="Add a child" />
              ) : (
            ...
            <div className="rounded-3xl border border-[var(--border)] bg-[var(--card)] p-6 shadow-sm">
              ...
              {upcoming.length === 0 ? (
                <EmptyState icon={Clock} title="No upcoming sessions" description="Book a session with a specialist to get started." />
              ) : (
                  ...
                  {upcoming.slice(0, 4).map((b, idx) => {
                    ...
                    return (
                      <li key={b.id ?? idx} className="rounded-2xl border border-[var(--border)] p-3">
                        ...
                        <p className="text-xs text-[var(--muted)] mt-1">{dt ? dt.toLocaleString() : 'Time TBD'}</p>
                        {b.status && (
                          <span className="inline-block mt-2 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded-full bg-orange-100 text-orange-700">{b.status}</span>
                        )}
                      </li>
                    )
                  })}
          ...
          <section className="rounded-3xl border border-[var(--border)] bg-[var(--card)] p-6 shadow-sm">
            ...
            {notes.length === 0 ? (
              <EmptyState icon={FileText} title="No notes yet" description="Notes from doctors and therapists will appear here." />
            ) : (
...
EmptyState.propTypes = { icon: PropTypes.any, title: PropTypes.string, description: PropTypes.string, cta: PropTypes.string }
